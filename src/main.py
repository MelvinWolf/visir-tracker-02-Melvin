...
def good_features(img):
    ...
    raw_corners = cv2.goodFeaturesToTrack(gray,25,0.01,10)
...
def face_detection_stream():
    ...
    old_frame, corners = good_features(old_frame)
    while(True):
        ret, frame = cap.read()
        frames += 1
        # Run the cascade face detector
        if(frames % 300 == 0):
            frame, corners = good_features(frame)
        ...
        # Wait for a second and print the number of frames 
        if elapsed_time >= 1:
            print (frames, "fps face detection")
            frames = 0
            start = end
